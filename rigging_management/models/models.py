...
user_roles = Table('user_roles', metadata,
    ...
    Column('role_id', Integer, ForeignKey('role.id'), primary_key=True, extend_existing=True)
)
...
rig_component_association = Table('rig_component_association', metadata,
    ...
    Column('component_id', Integer, ForeignKey('component.id'), primary_key=True, extend_existing=True)
)

class Role(db.Model):
    id = Column(Integer, primary_key=True)
    name = Column(String(80), unique=True, nullable=False, extend_existing=True)
